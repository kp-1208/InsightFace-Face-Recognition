...
while True:
    try:
            ...
            if len(bboxes) != 0:
                ...
                for bboxe in bboxes:
                    ...
                    if cos_similarity < cosine_threshold and proba > proba_threshold:
                        name = le.classes_[j]
                        text = "{}".format(name)
                        if text not in recognized_faces:
                            recognized_faces[text] = [proba, cos_similarity]
                            
                        else:
                            if recognized_faces[text][0] < proba:
                                recognized_faces[text][0] = proba
                            if recognized_faces[text][1] > cos_similarity:
                                recognized_faces[text][1] = cos_similarity
        ...
        df = pd.DataFrame(list(recognized_faces.items()), columns=['Name', 'Probability'])
